...
export NGINX_GCS_BUCKET=${NGINX_GCS_BUCKET}
...
if [ $NGINX_SUBFOLDER != 0 ]; then
  envsubst '${PHP_HOST} ${PHP_PORT} ${NGINX_DEFAULT_SERVER_NAME} ${NGINX_DEFAULT_ROOT} ${NGINX_SUBFOLDER} ${NGINX_SUBFOLDER_ESCAPED} ${NGINX_GCS_BUCKET}' < /templates/subfolder.conf > /etc/nginx/conf.d/default.conf
fi
...
for filename in /etc/nginx/conf.d/fragments/*.conf; do
  if [ -e "${filename}" ] ; then
    ...
    envsubst '${PHP_HOST} ${PHP_PORT} ${NGINX_DEFAULT_SERVER_NAME} ${NGINX_DEFAULT_ROOT} ${NGINX_SUBFOLDER} ${NGINX_SUBFOLDER_ESCAPED} ${NGINX_GCS_BUCKET}' < $filename.tmp > $filename
    rm ${filename}.tmp
  fi  
done
